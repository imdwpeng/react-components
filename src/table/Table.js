...
const pageDecorator = WrappedComponent => class extends Component {
...
class Table extends Component {
    ...
    handleStretchIng = (e) => {
        ...
        if (iWidth <= 40) {
            iWidth = 40;
            // 考虑左滚动距离
            iLeft = stretch.thLeft + delta.deltaX + 40;
        }
    ...
    render() {
        const {leftColumns, mainColumns, rightColumns, dataSource, ifScroll, trHeight, stretch, delta, sort, tableWidth, tableHeight, outWidth, outHeight} = this.state;
        const {affix, border, widthAuto, pagination, loading} = this.props;
        ...
        return (
            <div style={{height: '100%'}}>
                ...
                </div>
                {
                    pagination &&
                    pagination.total !== 0 &&
                    <Pagination {...pagination} onClick={this.props.onChangePage}/>
                }
            </div>
        )
    }
...
const FinalSelect = compose(pageDecorator)(Table);

class TableC extends Component {
    render() {
        return (
            <FinalSelect {...this.props}></FinalSelect>
        )
    }
}
